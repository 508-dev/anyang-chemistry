...
#     -> apps/web/android/app/build.gradle         versionName + versionCode
...
# because F-Droid's update bot regex-parses them out of build.gradle to
# decide there is a new version to build — an expression it cannot parse means
# no automatic F-Droid releases.
#
# Usage:
...
gradle_file=apps/web/android/app/build.gradle
